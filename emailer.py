...
def build_html_email(jobs):
    ...
    <div style="max-width: 700px; margin: 0 auto; background: white;
      ...
      <div style="background: #0D2B45; padding: 28px 32px;">
        <h1 style="color: white; margin: 0; font-size: 22px;">📌 Ontario Project Coordinator &amp; Canada Remote Alert</h1>
        ...
        <p style="color: #D7F0FF; margin: 0 0 12px 0; font-size: 13px;">
          All Ontario coordinator roles, plus remote roles anywhere in Canada
        </p>
...
def send_email(jobs):
    ...
    msg["Subject"] = (
        f"📌 {len(jobs)} New Project Coordinator Role{'s' if len(jobs) != 1 else ''}"
